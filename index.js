cat > index.js <<'EOF'
require("dotenv").config();

const express = require("express");
const http = require("http");
const { Server } = require("socket.io");
const mongoose = require("mongoose");

const auth = require("./auth");

const app = express();
const server = http.createServer(app);
const io = new Server(server);

app.use(express.static("public"));
app.use(express.json());

mongoose.connect(process.env.MONGODB_URI)
.then(()=>console.log("MongoDB connected"))
.catch(console.log);

/* ================= GAME ================= */
let game = {
  phase: "waiting",
  players: {},
  selected: {},
  called: [],
  takenCards: [],
  interval: null,
  gameId: 0,
  lock: false
};

/* ================= CARD ================= */
function generateCard() {
  function r(min, max) {
    let a = [];
    while (a.length < 5) {
      let n = Math.floor(Math.random() * (max - min + 1)) + min;
      if (!a.includes(n)) a.push(n);
    }
    return a.sort((a, b) => a - b);
  }

  const B = r(1, 15),
        I = r(16, 30),
        N = r(31, 45),
        G = r(46, 60),
        O = r(61, 75);

  return [
    [B[0], I[0], N[0], G[0], O[0]],
    [B[1], I[1], N[1], G[1], O[1]],
    [B[2], I[2], "FREE", G[2], O[2]],
    [B[3], I[3], N[3], G[3], O[3]],
    [B[4], I[4], N[4], G[4], O[4]]
  ];
}

/* ================= FIXED 600 CARDS ================= */
const globalCards = [...Array(600)].map(() => generateCard());

/* ================= WIN CHECK ================= */
function isWinner(card) {

  // ROW
  for (let r = 0; r < 5; r++) {
    if (card[r].every(n => n === "FREE" || game.called.includes(n))) {
      return true;
    }
  }

  // COLUMN
  for (let c = 0; c < 5; c++) {
    let ok = true;
    for (let r = 0; r < 5; r++) {
      let n = card[r][c];
      if (n !== "FREE" && !game.called.includes(n)) ok = false;
    }
    if (ok) return true;
  }

  // DIAGONAL
  let d1 = true;
  let d2 = true;

  for (let i = 0; i < 5; i++) {
    let a = card[i][i];
    let b = card[i][4 - i];

    if (a !== "FREE" && !game.called.includes(a)) d1 = false;
    if (b !== "FREE" && !game.called.includes(b)) d2 = false;
  }

  return d1 || d2;
}

/* ================= SOCKET ================= */
io.on("connection", (socket) => {

  socket.on("join", (data) => {

    const approved = auth.isApproved(data.phone);

    game.players[data.phone] = {
      socketId: socket.id,
      telegramName: data.telegramName,
      cards: globalCards,
      approved
    };

    socket.emit("approved", approved);
    socket.emit("game_id", game.gameId);
    socket.emit("cards", globalCards);
    socket.emit("phase", game.phase);
    socket.emit("called", {
      list: game.called,
      gameId: game.gameId
    });
    socket.emit("taken", game.takenCards);
  });

  socket.on("select_cartelas", (data) => {

    const user = game.players[data.phone];

    if (!user || !user.approved) {
      return socket.emit("msg", "❌ PAY FIRST");
    }

    if (game.phase !== "picking") {
      return socket.emit("msg", "⏳ WAIT FOR NEXT GAME");
    }

    let chosen = [];

    for (let card of data.cards) {
      let str = JSON.stringify(card);

      // Already taken by another player
      if (game.takenCards.includes(str)) {
        socket.emit("msg", "ALREADY TAKEN");
        continue;
      }

      game.takenCards.push(str);
      chosen.push(card);
    }

    if (chosen.length === 0) {
      return socket.emit("msg", "NO AVAILABLE CARTELAS");
    }

    game.selected[data.phone] = {
      ...user,
      chosen
    };

    io.emit("taken", game.takenCards);
  });
});

/* ================= PICK PHASE ================= */
function startPickPhase() {

  game.phase = "picking";
  game.called = [];
  game.selected = {};
  game.takenCards = [];
  game.gameId++;
  game.lock = false;

  io.emit("phase", "picking");
  io.emit("called", {
    list: [],
    gameId: game.gameId
  });
  io.emit("taken", []);
  io.emit("game_id", game.gameId);
  io.emit("reset_board");
  io.emit("stop_audio", game.gameId);

  let t = 30;

  const timer = setInterval(() => {
    io.emit("countdown", t);
    t--;

    if (t < 0) {
      clearInterval(timer);

      // Start only if at least one approved player selected cartelas
      if (Object.keys(game.selected).length === 0) {
        return startPickPhase();
      }

      startGame();
    }
  }, 1000);
}

/* ================= GAME ================= */
function startGame() {

  game.phase = "playing";
  io.emit("phase", "playing");

  game.interval = setInterval(() => {

    if (game.lock) return;
    game.lock = true;

    // All 75 numbers already called
    if (game.called.length >= 75) {
      clearInterval(game.interval);
      endGame("NO WINNER", null);
      return;
    }

    let n;
    do {
      n = Math.floor(Math.random() * 75) + 1;
    } while (game.called.includes(n));

    game.called.push(n);

    // Send number first (client plays voice)
    io.emit("number", {
      value: n,
      gameId: game.gameId
    });

    // Then update called numbers after short delay to sync with voice
    setTimeout(() => {
      io.emit("called", {
        list: game.called,
        gameId: game.gameId
      });
      game.lock = false;
      checkWinner();
    }, 700);

  }, 3000);
}

/* ================= WINNER ================= */
function checkWinner() {

  for (let phone in game.selected) {
    const player = game.selected[phone];

    for (let card of player.chosen) {
      if (isWinner(card)) {

        clearInterval(game.interval);

        io.emit("winner", {
          telegramName: player.telegramName,
          card
        });

        endGame(player.telegramName, card);
        return;
      }
    }
  }
}

/* ================= END GAME ================= */
function endGame(name, card) {

  game.phase = "waiting";

  if (name === "NO WINNER") {
    io.emit("game_end", "🏁 NO WINNER");
  } else {
    io.emit("game_end", "🏆 WINNER: " + name);
  }

  // Stop current voices immediately
  io.emit("stop_audio", game.gameId);

  // Clear after 1 second
  setTimeout(() => {

    game.called = [];
    game.takenCards = [];
    game.selected = [];
    game.lock = false;
    game.gameId++;

    io.emit("called", {
      list: [],
      gameId: game.gameId
    });

    io.emit("taken", []);
    io.emit("reset_board");

    // Show final winner card
    if (card && name !== "NO WINNER") {
      io.emit("winner_final", {
        name,
        card
      });
    }

  }, 1000);

  // Start next game automatically after 30 seconds
  setTimeout(() => {
    startPickPhase();
  }, 30000);
}

/* ================= AUTO START ================= */
setTimeout(() => {
  startPickPhase();
}, 2000);

/* ================= SERVER ================= */
server.listen(process.env.PORT || 10000, () => {
  console.log("🚀 FINAL CLEAN BINGO READY");
});
EOF